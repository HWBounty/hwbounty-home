...
export const LoginPopup = (props) => {
    ...
    return (
        <div style=
            ...
            } onKeyDown={(key) => key.code === "Enter" && document.getElementById("loginButton").click()}>
            {showProgress && <LinearProgress />}
            <img src="https://raw.githubusercontent.com/HWBounty/HWBountyAssets/2a379c8357790b232ac7b17b7de2587d4967a0a1/HopperEyes.svg" style={{ width: "7.5rem", borderRadius: "10000ch" }} />
            <Typography className={`${classes.hwbountyTitle}`}>Sign In </Typography>
            <Typography className={`${classes.subText}`}>Use your HWBounty Account</Typography>
            <br /><br />
            <TextField label="Email Address" variant="outlined" fullWidth onChange={onTextChange} id="email" error={!!loginFailed} helperText={loginFailed && "Invalid email or password"} disabled={!typingAllowed} />
            <span style={{ marginTop: "1.25rem" }} />
            <TextField label="Password" variant="outlined" fullWidth type={showPassword ? "all" : "password"} onChange={onTextChange} id="password" error={!!loginFailed} helperText={loginFailed && "Invalid email or password"} disabled={!typingAllowed} />
            <span style={{
                width: "100%",
            }}>
                <Button className={`${classes.secondaryOptionButton} ${classes.disableHoverEffect}`} disabled>Forgot password?</Button>
            </span>
            <br />
            <br />
            <span style={{
