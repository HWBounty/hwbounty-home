...
let ProgressBarMusic = (props) => {
	...
	return (
		<div>
			<Slider value={Math.round(document.getElementById("streamingVideoForMusic").currentTime * 100 / document.getElementById("streamingVideoForMusic").duration)} onChange={changeTime} />
			<Typography variant="body2" color="textSecondary" style={{
				display: 'inline-block',
			}}>{`${parseSecsToEnglish(document.getElementById("streamingVideoForMusic").currentTime )} | ${parseSecsToEnglish(document.getElementById("streamingVideoForMusic").duration )}`}</Typography>
		</div>
