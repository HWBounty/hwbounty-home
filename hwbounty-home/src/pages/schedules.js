...
export const ScheduleCatalog = (props) => {
  ...
  if (schedulesSearch && lschedulesSearch !== schedulesSearch) {
    ...
    (async () => {
      let res = await axios
        .get(`https://api.hwbounty.help/schedules/search/${schedulesSearch}`)
        .catch(console.trace);
  ...
  return (
    <DesktopLayout
      ...
      setSchedulesSearch={setSchedulesSearch}
...
const ScheduleItem = (props) => {
  ...
  /*
	...
	*/
  return (
    <Paper className={`${classes.scheduleDisplay}`}>
      <Tabs
        variant="fullWidth"
        centered
        scrollButtons="auto"
    ...
    <div>
      <div className={`${classes.catalogLeft}`}>
        ...
        <TextField
          id="searchBox"
          variant="outlined"
          label="What school are you from?"
          className={`${classes.catalogLeftSearchBox}`}
